...
namespace ProfitOptimizer
{
    static class Processor
    {
            ...
            while (orders.Count > 0)
            {
                ...
                switch (orders[0].ProductType)
                {
                    case ProductTypes.GYB:
                        ...
                        break;
                    case ProductTypes.SB:
                        if (PreviousProductType == ProductTypes.FB) ElapsedMinutes += 13;
                        break;
                }
                ...
                switch (orders[0].ProductType)
                {
                    ...
                    case ProductTypes.SB:
                        ...
                        StartSchedule[2] = ElapsedMinutes + 21;
                        StartSchedule[3] = ElapsedMinutes + 31;
                        StartSchedule[4] = ElapsedMinutes + 36;
                        StartSchedule[5] = ElapsedMinutes + 51;
                        EndSchedule[0] = ElapsedMinutes + orders[0].TimeToFinishCutting;
                    ...
                    case ProductTypes.FB:
                        ...
                        StartSchedule[2] = ElapsedMinutes + 24;
                        StartSchedule[3] = ElapsedMinutes + 36;
                        StartSchedule[4] = ElapsedMinutes + 41;
                        StartSchedule[5] = ElapsedMinutes + 61;
                        EndSchedule[0] = ElapsedMinutes + orders[0].TimeToFinishCutting;
                ...
                Program.FirstOutput[index][3] = ConvertToDate(ElapsedMinutes);
                Program.FirstOutput[index][4] = ConvertToDate(CompleteTime);
                Program.FirstOutput[index][5] = ConvertToDate(orders[0].TimeLeft);

                Program.SecondOutput.AddRange(GetScheduleData(orders[0].ProductType, orders[0].Identifier, orders[0].Pieces));
        ...
        private static List<string[]> GetScheduleData(ProductTypes productType,string Identifier,int pieces)
        {
                    ...
                    for (int j = 0; j <= daysElapsed; j++)
                    {
                        ...
                        else if (WorkStations[i][counter] == "Hajlító-2")
                        {
                            ...
                            else if (j == 0 && j == daysElapsed)
                            {
                                ...
                                {
                                    output.Last()[2] = ConvertToDate(StartSchedule[i]).Split(' ')[1];
                                    output.Last()[3] = ConvertToDate((EndSchedule[i] - (productType == ProductTypes.GYB ? 10 : (productType == ProductTypes.SB ? 15 : 16)))).Split(' ')[1];
                                }
                            }
                            else if (j == 0)
                            {
                        ...
                        else if (WorkStations[i][counter] == "Hegesztő-1")
                        {
                        ...
                        else if (WorkStations[i][counter] == "Hegesztő-2")
                            {
                                ...
                                else if (j == 0 && j == daysElapsed)
                                {
                                    ...
                                    {
                                        output.Last()[2] = ConvertToDate(StartSchedule[i]).Split(' ')[1];
                                        output.Last()[3] = ConvertToDate((EndSchedule[i] - (productType == ProductTypes.GYB ? 8 : (productType == ProductTypes.SB ?10 : 12)))).Split(' ')[1];
                                    }
                                }
                                else if (j == 0)
                                {
